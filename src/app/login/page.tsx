...
export default function Login() {
  ...
  return (
    <main className="flex-1 w-full min-h-screen grid lg:grid-cols-2">

      {/* ═══════════════ LEFT PANEL — dark hero style ═══════════════ */}
      <div
        className="hidden lg:flex flex-col justify-center px-14 py-16 text-white relative overflow-hidden"
        style={leftPanelStyle}
      >
        {/* Subtle radial glow */}
        <div className="absolute bottom-0 right-0 w-[60%] h-[40%] pointer-events-none"
          style={{ background: "radial-gradient(ellipse at 100% 100%, rgba(49,92,255,.12), transparent 70%)" }} />
        ...
        {/* Status pill */}
        <div className="inline-flex items-center gap-[9px] mb-10 px-3.5 py-2 border rounded-full text-[0.82rem] font-extrabold w-fit relative z-10" style={statusPillStyle}>
        ...
        {/* Headline */}
        <h1 className="font-display font-extrabold text-[clamp(2.2rem,3.5vw,3rem)] leading-[1.05] tracking-tight mb-[18px] max-w-[440px] relative z-10">
          Sign in to manage your queue.
        </h1>
        <p className="text-[1.05rem] leading-[1.7] max-w-[400px] mb-[42px] relative z-10" style={{ color: "rgba(255,255,255,.68)" }}>
      ...
      <div className="flex items-center justify-center px-6 sm:px-10 py-12" style={{ background: "linear-gradient(180deg,#f5f8fd 0%,#fff 100%)" }}>
